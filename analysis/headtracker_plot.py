...
# USER CONFIGURATION 
dev = HID.Connect("/dev/hidraw4")   # Device path
...
ax[0].set_axis_bgcolor(light_gray)
ax[1].set_axis_bgcolor(light_gray)
ax[2].set_axis_bgcolor(light_gray)
# Remove ticklabels
...
def init(): 
    ...
    ax[2].set_ylim([-32768*MAG_RES/80, 32767*MAG_RES/80])
    for line in lines:
        line.set_data([], []) 
    return tuple(lines)
