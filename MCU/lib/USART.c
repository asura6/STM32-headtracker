...
void USART_Send_Hex(uint32_t value) {
    USART_Send_Str("0x");
    if (value == 0) {
        USART_Send_Str("00000000");
    } else if (value == 1) {
        USART_Send_Str("00000001");
    }

    char buffer[9];
    uint8_t i = 0;
    uint32_t exp = 0x10000000;
...
void USART_Send_Int(int32_t value) { 
    ...
    if (value & mask) {
        USART_Send_Str("-");
        USART_Send_Integer(-1*value);
    } else {
        USART_Send_Str(" ");
        USART_Send_Integer(value);
    } 
}
