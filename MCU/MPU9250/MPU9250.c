...
void Init_MPU9250(void) {
    /* Reset all registers */
    MPU9250_Reset(); 
    /* Enable bypass-mode to access magnetometer as well as latching of the
     * interrupt pin which mean the interrupt must be cleared manually */
    MPU9250_Write_Byte(MPU6050_ADDR, MPU6050_RA_INT_PIN_CFG, 0x22U);
    MPU9250_Write_Byte(MPU6050_ADDR, MPU6050_RA_INT_PIN_CFG, 0x22U);
...
void Init_AK8963(void) {
    ...
    MPU9250_cal_vals.mag[0] = (float)((buffer[0]-128)/256 + 1);
    MPU9250_cal_vals.mag[1] = (float)((buffer[1]-128)/256 + 1);
    MPU9250_cal_vals.mag[2] = (float)((buffer[2]-128)/256 + 1);
...
void MPU6050_Poll_Axis() { 
    ...
    MPU9250_axes.x.accel = ((buffer[0] << 8) | buffer[1]);
    MPU9250_axes.y.accel = ((buffer[2] << 8) | buffer[3]);
    MPU9250_axes.z.accel = ((buffer[4] << 8) | buffer[5]);
    /* Throw away temperature measurement */
    MPU9250_axes.x.gyro = ((buffer[8] << 8) | buffer[9]);
    MPU9250_axes.y.gyro = ((buffer[10] << 8) | buffer[11]);
    MPU9250_axes.z.gyro = ((buffer[12] << 8) | buffer[13]);
}
...
void MPU9250_Fusion(void) {
    ...
    float mx = ((float)MPU9250_axes.y.mag)*(MAG_RES)*MPU9250_cal_vals.mag[0];
    float my = ((float)MPU9250_axes.x.mag)*(MAG_RES)*MPU9250_cal_vals.mag[1];
    float mz = ((float)MPU9250_axes.z.mag)*(MAG_RES)*MPU9250_cal_vals.mag[2]; 
    /* Sensor fusion */
    MadgwickAHRSupdate(gx, gy, gz, ax,  ay,  az,  mx,  my,  mz); 
    /* Quaternion to Tait-Bryan angles */
    angles[0] = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + 
            q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);   
    angles[1] = asin(2.0f * (q[1] * q[3] - q[0] * q[2]));
    angles[2]  = atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - 
            q[1] * q[1] - q[2] * q[2] + q[3] * q[3]);

    angles[0] *= 4*32768.f/PI; //45 degrees resolution
    angles[1] *= 32768.f/PI; //180 degrees resolution
    angles[2] *= 32768.f/PI; //180 degrees resolution
